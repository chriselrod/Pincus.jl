using Revise, Pincus, BenchmarkTools

f(x) = cos(40π*x) + (1 - (2π*x-π)^2)
f(x::Float32) = cos( Cuint(40)x*π ) + ( Cuint(1) - abs2(Cuint(2)x*π-π) )

gh = gauss_hermite_tup(200, BigFloat);
uniboxmaximize(f, gh)
@benchmark uniboxmaximize($f, $gh)
threaded_uniboxmaximize(f, gh)



function rosenbrock2(x::NTuple{2,T}) where T
    -abs2(one(T) - x[1]) - T(100.0) * abs2(x[2] - abs2(x[1]))
end
...
td = TwiceDifferentiable(rosenbrock, initial_x; autodiff = :forward);
result = optimize(td, initial_x, BFGS())
td.f_calls = [0]; td.df_calls = [0]; td.h_calls = [0];
result = optimize(td, initial_x, NewtonTrustRegion())
